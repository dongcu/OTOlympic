...
<body>
	...
	<div class="banana">
		<div class="board_body">
			...
			<!-- 본문 -->
			<div class="view_content">${ex_view.ex_content}</div>
			<div class="btns">
				<a class="btn_back" href="/profile/my_ex?num=${select}">목록</a>

				<!-- 수정 / 삭제버튼 표시 -->
				<c:if test="${ex_view.ex_writer == loginUser.userid}">
					<a class="btn_modi" href="/profile/my_ex_modify?ex_num=${ex_view.ex_num}">수정</a>
			...
			<ul class="reply_textbox">
				<form method="post" action="/profile/my_ex_detail_reply_write" name="ex_replyform" id="ex_replyform">
						...
						<div class="free_reply_div1 reply${ex_reply.exc_num}">
							...
							<div class="free_reply_div2">
								...
								<c:if test="${ex_reply.exc_writer == loginUser.userid}">
									...
									<form method="post" action="${cp}/profile/my_ex_detail_reply_delete?ex_num=${ex_view.ex_num}&exc_num=${ex_reply.exc_num}" id="ex_replydelete" name="ex_replydelete">
...
<script>
	...
	function updateviewBtn(e) {
		...
		$.ajax({
			url : "my_ex_detail_reply_detail",
			type : "POST",
			data : {"exc_num" : exc_num},
			success : function(result) {
				replyModi.empty();
				var commentsview = "";
				commentsview += '<form action="/profile/my_ex_detail_reply_modify" name="replymodiform" id="replymodiform" method="post">';
