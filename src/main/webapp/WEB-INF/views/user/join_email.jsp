			...
			<form name="email_certification" id="emailform" method="post" action="${path}/user/join_form">
				<table>
					<tr>
						<th>이메일</th>
						<td>
							<input class="box" id="email_id" name="email_id" placeholder="이메일" /> @
							<input class="box" id="domain-txt" name="domain-txt" type="text" />
							...
							<input type="button" id="mail-Check-Btn" value="전송" onclick="">
						</td>
						<td>
					...
					<tr>
						<td></td>
						<td style="color: gray; line-height: 16px;">
							*이메일 전송까지는 30초~1분 정도의 시간이 소요됩니다.
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					...
					<tr>
						<td></td>
						<td style="color: gray; line-height: 16px;">
							*'인증번호가 전송되었습니다.'라는 메시지가 출력되면 입력창이 활성화됩니다.
					...
					<tr>
						<td></td>
						<td style="color: gray; line-height: 16px;">
							*인증번호는 30분 내에 입력해야 하며, 30분이 초과될 시 로그인 화면으로
							<br>&nbsp; 돌아갑니다.
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2" style="text-align: center;">
							<input type="submit" value="인증완료" id="certification_pass">
						</td>
					</tr>
				</table>
			</form>
...
//   메일 가져오기
	...
	$('#mail-Check-Btn').click(function() {
		...
		else if (!mailid.test($('#email_id').val())) {
			alert('이메일을 확인해주세요.');
			$('#email_id').focus();
		}
		else if (!maildomain.test($('#domain-txt').val())) {
			alert('이메일을 확인해주세요.');
			$('#domain-txt').focus();
		}
		else {
